...
class MusicLibraryController
  ...
  def list_songs
    Song.all.sort{|a, b| a.name <=> b.name}.each.with_index(1){|song, index| puts "#{index}. #{song}"} 
  ...
  def list_artists
    sorted_artists = Artist.all.sort{|a, b| a <=> b}.each.with_index(1){|song, index| puts "#{index}. #{song}"} 
      artist.name
    end
