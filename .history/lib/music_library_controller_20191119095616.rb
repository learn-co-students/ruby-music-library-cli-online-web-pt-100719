...
class MusicLibraryController
  ...
  def list_artists
    Artist.all.sort{|a, b| a.name <=> b.name}.each_with_index{|song, index| puts "#{index + 1}. #{song.name}"}
  ...
  def list_songs_by_artist
    ...
    if artist = Artist.find_by_name(artist_input)
      artist.songs.sort{|a, b| a.name <=> b.name}.each_with_index(1) do |song, index|
  ...
  def list_songs_by_genre
    ...
    if genre = Genre.find_by_name(genre_input)
      genre.songs.sort{|a, b| a.name <=> b.name}.each_with_index(1) do |song, index|
  ...
  def play_song
    ...
    if list_songs.include?(song_input)
      song = list_songs[song_input]
